...
class FuncionarioCreateSerializer(serializers.Serializer):
    ...
    password = serializers.CharField(write_only=True, required=False)  # ADICIONE required=False  

    def validate_cpf_cnpj(self, value):
        instance = self.context.get('instance')  # pega o funcionário sendo editado
        if User.objects.filter(cpf_cnpj=value).exclude(id=instance.user.id if instance else None).exists():
